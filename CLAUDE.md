...
**核心策略**：策略启动时立即市价开仓 → 挂限价止盈单(1%) → 价格下跌触发加仓(1.5倍递增) → 止盈/止损后自动重开首单。
...
### 马丁网格交易流程
1. 调度器每 30 秒触发策略执行
2. **无持仓** → 市价开首单 + 挂限价止盈单 + 挂第一层加仓限价单
3. **有持仓** → 更新标记价格和浮亏 → 依次检查：
   - 止盈单是否成交 → 成交则平仓记录 + 自动重开首单
   - 加仓单是否成交 → 成交则记录新持仓 + 取消旧止盈 + 挂新止盈 + 挂下一层加仓单
   - 累计浮亏是否达到止损阈值 → 触发则平仓 + 自动重开首单
...
- **最大加仓层数** `max_layers`：默认 8
- **累计亏损阈值** `cumulative_loss_threshold_u`：单策略累计浮亏U数，0=禁用
- **平仓后重开** `reopen_after_close`：止盈/止损后是否自动开新首单
...
- **加仓限价单**：每层加仓以限价单挂出，成交后自动挂下一层。使用首单入场价计算所有层级触发价，避免网格漂移。
