...
**核心策略**：策略启动时立即市价开仓 → 挂限价止盈单(1%) → 单层链式加仓限价 → 止盈全平后可配置自动重开首单；交易所止损触发后全部平仓并自动重开首单。
...
- **止损触发亏损** `cumulative_loss_threshold_u`：按「当前整仓」推算交易所止损触发价（名义亏损达到约 U）；**0=不挂止损**；首仓开仓即挂止损，每次加仓后更新；触发后全平并自动重开首单
- **止盈全平后重开** `reopen_after_close`：仅当 **止盈限价全部平仓** 后是否自动市价重开首单
...
- **交易所止损**：触发价由亏损阈值 U 与整仓推算；首仓即挂全仓条件单，加仓后撤旧挂新；条件单检测走 algo API + 交易所空仓推断兜底；触发后全平记账并自动重开首单（不依赖 `reopen_after_close`）。
